...
from fosi.base import Assert,IndexWrapper,Registry
from fosi.type import Role,Type
from fosi.entity import Port,Generic,Entity
from fosi.signal import Signal
...
class Environment():
  ...
  def _init_builtin(self):
     ...
     Port(self, 'start', Role.Output, self.fosi.t_logic)
     Port(self, 'ready', Role.Input, self.fosi.t_logic,
                                     self.generics.lookup('ReadyCount'))
     Port(self, 'regPorts', Role.Master, self.fosi.t_regPort,
                                     self.generics.lookup('RegPortCount'))
     Port(self, 'hmem', Role.Slave, self.fosi.t_axi)
     Port(self, 'cmem', Role.Slave, self.fosi.t_axi)
     Port(self, 'nvme', Role.Slave, self.fosi.t_ctrl)
     Port(self, 'int1', Role.Slave, self.fosi.t_hs)
     Port(self, 'int2', Role.Slave, self.fosi.t_hs)
     Port(self, 'int3', Role.Slave, self.fosi.t_hs)
     self.s_ctrlRegs = Signal(self.fosi, 'ctrlRegs', self.fosi.t_regPort)
...
class FOSI():
  ...
  def _resolve_generic(self, key, value):
    m = FOSI.GenericPattern.match(key)
    if m:
      name = m.group(1)
      return (name, value)
    return None
  ...
  def _resolve_port_definition(self, key, value):
    m = FOSI.PortRolePattern.match(key)
    if m:
  ...
  def _resolve_port_connect(self, key, value, index=None):
    m = FOSI.PortConnectPattern.match(key)
    if m:
      name = m.group(1)
      if isinstance(value, str):
  ...
  def _resolve_property(self, key, value):
    m = FOSI.PropertyPattern.match(key)
    if m:
      name = m.group(2)
      if m.group(1) == 't':
