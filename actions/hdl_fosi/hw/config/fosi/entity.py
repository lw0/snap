...
class Port():
  ...
  def instantiate(self, instance):
    ...
    if inst.is_vector():
      ...
      inst.unpack_signal = Signal(inst.fosi, unpack_signal_name, inst.type.base())
    return inst
...
class Entity():
  def __init__(self, fosi, name, **props):
    self.fosi = fosi
    self.name = name
    self.fosi.entities.register(self.name, self)
  ...
  def instantiate(self, name):
    ...
    inst.name = name
    inst.fosi.instances.register(inst.name, inst)
    inst.identifier = 'i_{}'.format(inst.name)
    inst.fosi.identifiers.register(inst.identifier, inst)
